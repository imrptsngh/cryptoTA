...
import save_historical_data_new
...
def run():
    ...
    print("\n\n---------------------------------------------------------\n\n")
    print("Hello and Welcome to the Crypto Trader Bot")
    time.sleep(5)
...
def scalping_orders(coin, wait=1, tot_time=1):
    ...
    ap, aq, bp, bq, place_ask_order, place_bid_order, spread, proj_spread, max_bid, min_ask = visualize_market_depth(
        wait, tot_time, coin)
    print("Coin: {}\nPrice to Place Ask Order: {}\nPrice to place Bid Order: {}".format(coin, place_ask_order,
                                                                                        place_bid_order))
...
def visualize_market_depth(wait_time_sec='1', tot_time='1', sym='ICXBNB', precision=5):
    ...
    for i in range(1, int(cycles) + 1):
        ...
        plt.plot([price, price], [0, max_quant], color='green',
                 label='Price - Cycle: {}'.format(i))  # Vertical Line for Price
    ...
    ax.set(xlabel='Price', ylabel='Quantity',
           title='Binance Order Book: {} \n {}\n Cycle Time: {} seconds - Num Cycles: {}'.format(sym, start_time,
                                                                                                 wait_time_sec, cycles))
    plt.legend()
    plt.show()
